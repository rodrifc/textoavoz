	...
	for (let file of event.target.files) {
		...
		if (file) {
			...
			reader.onload = () => {
				...
				} else if ( file_name_toLowerCase.endsWith('.docx') ) {
					convertDocxToTxt(file).then(result => get_text(file.name.slice(0, file.name.lastIndexOf(".")), result, true))
				} else if ( file_name_toLowerCase.endsWith('.pdf') ) {
					convertPdfToTxt(file).then(result => get_text(file.name.slice(0, file.name.lastIndexOf(".")), result, true))
				}
...
const start = () => {
	save_settings()
	save_path_handle = null // Aseguramos que se salte la selección de carpeta
	get_audio()
}
